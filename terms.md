---
permalink: /terms/
title: SlabIQ Terms of Use
---

# SlabIQ Terms of Use

**Last updated: 26 September 2026**

These Terms of Use ("Terms") are an agreement between you and Jake Murphy, a
sole trader in Queensland, Australia, trading as SlabIQ ("we", "us"). They
cover your use of the SlabIQ app and the online services it uses (together,
"SlabIQ"). By downloading or using SlabIQ you agree to these Terms. If you
don't agree, please don't use the app.

Our [Privacy Policy](https://mangomate.github.io/slabiq-privacy/) explains how
we handle your information. It forms part of these Terms.

---

## 1. What SlabIQ does

SlabIQ helps concreters estimate slab and foundation jobs. It can read a plan
you upload, identify zones, dimensions, beams and specifications, calculate
quantities (concrete, mesh, reinforcement, edge and internal beams, footings
and similar items), apply your rates, and produce quotes and invoices.

SlabIQ is a **business tool** for people in the construction industry. You
must be 18 or over to use it.

## 2. Estimates must be checked before you rely on them

**SlabIQ gives computer-generated estimates, not guarantees.** AI plan reading
and automated takeoffs can misread drawings, pick up the wrong figure, miss
details or scope, or make calculation errors. The app shows confidence notes
and warnings where it can, but it won't catch every mistake.

**You are responsible for checking every quantity, dimension, rate and total
against the actual plans, the engineering and the site** before you order
materials, send a quote, sign a contract or start work. SlabIQ supports a
competent estimator's judgment. It doesn't replace it, and it isn't
engineering, design or professional advice.

To the extent the law allows, we are not responsible for losses that come from
relying on an estimate you haven't checked. That includes over- or
under-ordering concrete or materials, under-quoting, or disputes with clients.

## 3. Free use, subscriptions and billing

**Paid subscriptions are coming soon.** The parts of this section about paid
plans apply once they launch.

| Plan | Price | Includes |
|---|---|---|
| **Free** | Free | 3 free estimates |
| **Pro** | A$129 a month | 35 AI plan scans a month |
| **Max** | A$229 a month | 250 AI plan scans a month |

Every plan is subject to the fair-use limits in section 4.

- **What counts as a scan.** One plan successfully processed is one scan.
  Reads that fail or whose result can't be used, page previews, Find Scale
  and price-sheet imports don't count. Re-scanning the same plan page in the
  same estimate is free while that estimate has been scanned within the last
  3 months. A different page or a different plan file counts as a new scan.
  Editing the job description doesn't. Monthly allowances start again at the
  beginning of each calendar month, counted in UTC (10am on the 1st in
  Queensland).
- **Subscriptions.** Pro and Max are auto-renewing subscriptions sold through
  the Apple App Store or Google Play. The app shows each plan's price, billing
  period and inclusions before you buy. The price shown by the store at
  purchase is the price that applies.
- **Scan packs.** Where offered, extra AI scans can be bought as a one-off
  in-app purchase.
- **Auto-renewal.** A subscription renews automatically at the end of each
  period unless you cancel it at least 24 hours before the period ends. Your
  store account is charged for the renewal within the 24 hours before the
  period ends.
- **Free trials.** If a free trial is offered, it turns into a paid
  subscription when it ends unless you cancel first. Any unused trial time is
  forfeited when you buy a subscription.
- **Managing and cancelling.** You can manage or cancel your subscription in
  your App Store or Google Play account settings. Deleting the app does
  **not** cancel it.
- **Refunds.** Payments are processed by Apple or Google, and refunds are
  handled under their policies. Nothing in these Terms limits your rights
  under the Australian Consumer Law.
- **Price changes.** We may change subscription prices. The store will tell
  you before a change applies to your subscription, as its rules require.

## 4. Fair use and availability

AI scanning costs us real money per scan, so SlabIQ has fair-use limits on
every plan:

- a monthly scan allowance for each plan (see section 3);
- a daily AI usage limit for each device, set well above normal use. Every
  plan has one, including Max;
- a daily limit on Find Scale reads;
- hourly limits on requests;
- limits for each internet connection, which can affect people who share a
  network (for example, an office or site Wi-Fi);
- a monthly limit on AI read attempts for each device;
- limits on repeated failed reads, such as the same plan failing again and
  again; and
- a daily capacity cap across the whole service.

We may also temporarily pause AI scanning for maintenance or safety. When
scanning is unavailable, your saved quotes still open and you can still build
estimates manually, though new calculations need an internet connection.

SlabIQ relies on third-party services such as Anthropic, Railway, RevenueCat,
Apple and Google. We aim to keep it running well, but we don't promise it
will always be available, uninterrupted or error-free.

## 5. Your content and responsibilities

- **You own your content.** That includes the plans, photos, price sheets, job
  descriptions, estimates and client details you create or upload.
- **Licence to process.** You give us a limited, non-exclusive licence to
  process the content you upload, only as needed to provide SlabIQ. This
  includes sending it to our AI provider as described in the Privacy Policy.
  We don't use your plans to train AI models.
- **Your rights to upload.** Upload only plans and documents you are allowed
  to use for pricing a job. Plans often include other people's personal
  information, and you are responsible for having a proper reason to handle
  it.
- **Backups.** Your quotes, clients and settings are stored on your device,
  not with us. We can't recover them if you lose your device, uninstall the
  app or clear its data. Use the app's backup export, and keep your own copies
  (for example, exported PDFs) of anything important.
- **Your quotes and invoices.** Documents you create in SlabIQ are yours to
  issue. You are responsible for their accuracy and for meeting your tax,
  GST, licensing and contract obligations.

## 6. Acceptable use

You must not:

- use SlabIQ unlawfully, or upload material you don't have the right to use;
- try to get around quotas, rate limits, paywalls or authentication, or share
  or extract the app's access credentials;
- reverse-engineer, copy, resell, scrape or build a competing service from
  SlabIQ or its outputs in bulk, except where the law allows;
- interfere with, overload or attack the service or its providers; or
- use SlabIQ to generate content that breaches Anthropic's usage policies.

We may suspend or limit access for an install that breaches these rules or
puts the service at risk.

## 7. Intellectual property

SlabIQ's software, calculation engine, design and branding belong to us or
our licensors. We give you a personal, non-transferable licence to use the
app for your own business while you comply with these Terms. For apps
downloaded from the Apple App Store, Apple's standard
[Licensed Application End User Licence Agreement](https://www.apple.com/legal/internet-services/itunes/dev/stdeula/)
also applies. If it conflicts with these Terms, these Terms prevail to the
extent Apple allows.

## 8. Australian Consumer Law

Our services come with guarantees that cannot be excluded under the
Australian Consumer Law. Nothing in these Terms excludes, restricts or
changes any right or remedy you have under that law, or under any other law
that can't be excluded.

## 9. Disclaimers and limits on liability

Subject to section 8, and to the extent the law allows:

- SlabIQ is provided "as is" and "as available", without warranties beyond
  those set out in these Terms;
- we are not liable for indirect or consequential loss, lost profits, lost
  data, or losses from relying on unchecked estimates (see section 2); and
- our total liability for any claim relating to SlabIQ is limited to the
  amount you paid us for SlabIQ in the 12 months before the claim.

If the services are not of a kind ordinarily acquired for personal, domestic
or household use, then for a failure to meet a consumer guarantee our
liability is limited, where the law allows, to supplying the services again
or paying the cost of having them supplied again.

## 10. Ending use

You can stop using SlabIQ at any time by cancelling any subscription and
deleting the app. We may suspend or end access if you seriously or
repeatedly breach these Terms, or if we stop offering SlabIQ. If we stop
offering SlabIQ, we'll give reasonable notice where we can. Sections 2, 5, 8
and 9 continue to apply after your use ends.

## 11. Changes to these Terms

SlabIQ is under active development, and these Terms will change as it does.
We'll update the "Last updated" date. For significant changes, we'll let you
know in the app before they take effect. If you keep using SlabIQ after a
change takes effect, you accept the updated Terms.

## 12. Governing law

These Terms are governed by the laws of Queensland, Australia. You and we
submit to the courts of Queensland and courts that can hear appeals from
them.

## 13. Contact

**Jake Murphy**, SlabIQ
Queensland, Australia
slabiqapp@gmail.com

---

*SlabIQ is not affiliated with Apple Inc., Google LLC, Anthropic PBC, Railway
Corp. or RevenueCat, Inc.*
